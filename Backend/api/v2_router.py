...
from simulation_core_v2.schedulers_v2 import run_simulation_v2, ADRRv2, RoundRobinV2, SJFV2
...
try:
    ...
    raise RuntimeError(f"[v2_router] Failed to load v2 model at '{_V2_MODEL_PATH}': {e}")
...
def _build_feature_df(data: list[dict]) -> pd.DataFrame:
    ...
    ordered_features = list(v2_model.feature_names_in_) if hasattr(v2_model, "feature_names_in_") else _MODEL_FEATURES
...
async def simulate_scheduling_v2(request: SimulationRequestV2):
    ...
    sim_processes = [
        ProcessV2(pid=int(row["pid"]), arrival_time=int(row["arrival_time"]), category=category_labels[i])
    ...
    if "RR" in request.algorithms:
        results.append(run_simulation_v2(RoundRobinV2, sim_processes, time_quantum=request.time_quantum))
    if "SJF" in request.algorithms:
        results.append(run_simulation_v2(SJFV2, sim_processes))

    if not results:
        raise HTTPException(status_code=400, detail="No valid algorithm specified. Use 'ADRR', 'RR', or 'SJF'.")
